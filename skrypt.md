...
Uwaga - skrypt nie jest do udostępniania ani w formie pliku ani w trakcie wykładu jako prezentacja. To są tylko notatki dla oosby prowadzącej. Można z nich coś będzie jeszcze wyciąć, dodać, zmienić treść zadanek, poprawić kolejność itp.
...
3 * 2  # => 6
9 / 2  # => 4.5
9 // 2 # => 4 (dzielenie całkowite) - trzeba się upewnić, że dzieci wiedzą, co to jest dzielenie całkowite, z resztą, potęgowanie i kolejność wykonywania działań, niech jakieś dziecko powie i dajmy mu cukierka czy coś
2 ** 3 # => 8 (potęgowanie)
...
## Zmienne
Wstęp: Jasio ma 5 jabłek i 3 gruszki. Ile razem owoców ma Jasio?

Zmienną możemy wyobrazić sobie jako pudełko/koszyk/opakowanie z naklejką, do którego wkładamy jakąś wartość.

Nazwa zmiennej musi być jednym ciągiem znaków (bez spacji). Nie może zawierać operatorów ani zaczynać się od liczb. Zawiera tylko znaki z alfabetu angielskiego (wielkie i małe litery).

Do pudełka możemy wkładać liczby całkowite, ułamki dziesiętne, napisy oraz prawdę lub fałsz.

Są to różne typy danych, które Python od siebie odróżnia.

```python=
jablka = 5 # W koszyku jest liczba 5, na naklejce jest napis 'jablka'. Jasio ma 5 jabłek.
gruszki = 3
```

Typy mają swoje własne nazwy w Pythonie:
- Liczby całkowite - `int`
- Liczby zmiennoprzecinkowe (ułamki dziesiętne) - `float`
...
```python=
jablka + gruszki # = 8, Jasio ma razem 8 owoców
...
## Stringi czyli napisy
Napisy to ciągi znaków w cudzysłowie (lub apostrofach - nie ma znaczenia), np. "mama", "Ala ma kota".

Na napisach możemy wykonywać dwie operacje:
- Dodawanie dwóch napisów
- Mnożenie napisu przez liczbę - zapytać dzieci co się stanie
...
Gdy chcemy sprawdzić ilość znaków, używamy `len()`, na przykład:
```python
len("napis") # => 5
...
Służy do wypisywania na ekran. Można wypisywać np. tekst, wartości zmiennych.

```python=
print(5) # => 5
print(3 + 5) # => 8
print("to jest napis") # => to jest napis

x = 8
print(x) # => 8

y = "to tez jest napis"
print(y) # => to tez jest napis
```
...
Wstęp: Jasio ma 5 jabłek i 3 gruszki. Których owoców ma więcej?
...
Bardziej sensowny przykład?: Jasio ma trochę jabłek. Jeśli liczba jabłek jest parzysta, to Jasio dostanie 2 jabłka od taty. Jeśli liczba jabłek jest nieparzysta, to Jasio dostanie 1 jabłko od taty. Ile jabłek dostanie Jasio?


```python=
jablka = input("Podaj, ile Jasio ma jabłek na początku: ")
if jablka % 2 == 0: # przy okazji pytamy, co to znaczy, że liczba jest parzysta i tłumaczymy, dlaczego tak robimy
    print("Jasio dostanie 2 jabłka")
if jablka % 2 != 0:
    print("Jasio dostanie 1 jabłko")
```
...
Jeśli chcemy jakąs instrukcję wykonać 5 razy, to do tej pory napisalibyśmy ją 5 razy. 
...
while x <= 10:
    print(x)
    x = x + 1 # przy okazji powiedzieć, co to w ogóle znaczy, bo przecież w matematyce jak ktoś napisze x = x + 1 to jest sprzeczność i herezja
# => 1 2 3 ... 10
```
...
# Zadania dodatkowe, prawdopodobnie do wywalenia - listy mogą być za trudne dla dzieciaków z 5 klasy
